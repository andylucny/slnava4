...
#model_select = 'GLEE-Plus (SwinL)'
model_select = 'GLEE-Lite (R50)' 

if model_select != 'GLEE-Plus (SwinL)':
    cfg_r50 = get_cfg()
    add_deeplab_config(cfg_r50)
    add_glee_config(cfg_r50)
    conf_files_r50 = 'GLEE/configs/R50.yaml'
    checkpoints_r50 = torch.load('GLEE/GLEE_R50_Scaleup10m.pth', weights_only=True) 
    ...
    checkpoints_swin = torch.load('GLEE/GLEE_SwinL_Scaleup10m.pth', weights_only=True) 
...
def segment_image(img, prompt_mode, categoryname, custom_category, expressiong, results_select, num_inst_select, threshold_select, mask_image_mix_ration, model_selection):
    ...
    if prompt_mode == 'categories' or prompt_mode == 'expression':
        ...
        results['masks'] = mask_results_list
...
def text2mask(img):
    ...
    img_input = { 'background': img }
    prompt_mode_select = 'expression'
    category_select = None # "COCO-80"
    custom_category = None
    input_expressiong = "walkway or road, not obstacle"
    results_select = ["mask"]
    num_inst_select = 1
    threshold_select = 0.1
    mask_image_mix_ration = 0.65
    results = segment_image(img_input, prompt_mode_select, category_select, custom_category,input_expressiong, results_select, num_inst_select, threshold_select, mask_image_mix_ration,model_select)
    visual = results['visual']
    visual = cv2.cvtColor(visual,cv2.COLOR_BGR2RGB)
    mask = (results['masks'][0]*255).astype(np.uint8)
    return mask, visual
